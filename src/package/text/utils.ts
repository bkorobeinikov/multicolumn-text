import { intersectRelative, IRect } from "../geometry";
import {
  IHTMLText,
  IHTMLTextParagraph,
...
export function addMediaToPage(page: IVirtualPage, mediaRect: IRect) {
  // normalize media rect to page

  page.columns.forEach((c) => {
    const mediaRectInC = intersectRelative(c.rect, mediaRect);

    if (mediaRectInC) {
      c.media.push({
        absoluteRect: {
          top: mediaRectInC.top + c.absoluteRect.top,
          left: mediaRectInC.left + c.absoluteRect.left,
          width: mediaRectInC.width,
          height: mediaRectInC.height,
        },
        rect: mediaRectInC,
      });
    }
